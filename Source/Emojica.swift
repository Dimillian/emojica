...
public final class Emojica {
    ...
    private var _spriteSheetCache: [String: UIImage] = [:]
...
extension Emojica {
    ...
    /// Replaces the standard emoji with custom emoji from the provided spritesheet and coordinate
    /// - parameter string:      The string to convert.
    /// - parameter spriteSheet: The name of the image containing your emoji.
    /// - parameter coords:      The coordinate of your emoji in the spritesheet.
    /// - returns:               An `NSAttributedString` with all the emoji replaced with custom images.
    public func convert(string: String, spriteSheet: String, coords: CGRect) -> NSAttributedString {
        let attributedString = NSAttributedString(string: string)
        return self.convert(string: attributedString, spriteSheet: spriteSheet, coords: coords)
    }

    /// Replaces the standard emoji with custom emoji from the provided image set.
    /// - parameter string:      The string to convert.
    /// - parameter spriteSheet: The name of the image containing your emoji. If any.
    /// - parameter coords:      The coordinate of your emoji in the spritesheet. If any.
    /// - returns:               An `NSAttributedString` with all the emoji replaced with custom images.
    public func convert(string attributedString: NSAttributedString, spriteSheet: String? = nil, coords: CGRect? = nil) -> NSAttributedString {
        let result = NSMutableAttributedString(attributedString: attributedString)

        if let sheet = spriteSheet, let frame = coords {
            return replaceString(from: result.string, spriteSheet: sheet, coords: frame) ?? result
        } else {
...
extension Emojica {
    ...
    fileprivate func replaceString(from name: String, spriteSheet: String, coords: CGRect) -> NSAttributedString? {
        var spriteSheetImage = _spriteSheetCache[spriteSheet]
        if spriteSheetImage == nil {
            spriteSheetImage = UIImage(named: spriteSheet)
            guard let spriteSheetImage = spriteSheetImage else { return nil }
            _spriteSheetCache[spriteSheet] = spriteSheetImage
        }
        let attachment = EmojicaAttachment()
        attachment.image = spriteSheetImage?.extractEmojiAt(coords: coords)
        attachment.resize(to: self.pointSize, with: self.font)
        return NSAttributedString(attachment: attachment)
    }
